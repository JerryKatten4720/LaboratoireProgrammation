...
public class Team {
    ...
    public double FoodMax { get; set; }
    public double Electricity { get; set; }
    public double ElectricityMax { get; set; }
    public double Wood { get; set; }
    public double WoodMax { get; set; }
    public double Metals { get; set; }
    public double MetalsMax { get; set; }
    ...
    public void UpdateResources() {
        double surplus;
        if (Energy > EnergyMax) {
            surplus = Energy - EnergyMax;
            Energy = EnergyMax;
            Caps += surplus / 10;
        }

        if (Food > FoodMax) {
            surplus = Food - FoodMax;
            Food = FoodMax;
            Caps += surplus / 10;
        }

        if (Electricity > ElectricityMax) {
            surplus = Electricity - ElectricityMax;
            Electricity = ElectricityMax;
            Caps += surplus / 10;
        }

        if (Wood > WoodMax) {
            surplus = Wood - WoodMax;
            Wood = WoodMax;
            Caps += surplus / 10;
        }

        if (Metals > MetalsMax) {
            surplus = Metals - MetalsMax;
            Metals = MetalsMax;
            Caps += surplus / 10;
        }
    }
